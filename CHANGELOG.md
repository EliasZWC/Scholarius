...
原来只能靠「逐块点击 → 选类型」，而原生把摘要切成十几块，
想把一整段标成摘要就要点十几次。

现在：**原始视图里横向拖选中文字 → 浮出「建立区域」按钮 → 选类型**，
选中的行区间一次写成一条 `textMark`。

#### ⚠️ 高亮改成**自己画**（这是「一闪而过」的真因）

用户反馈：「一闪而过，和之前随机框一样」。

实测发现：手势结束后选区**确实还在**
（`getSelection().toString().length === 47`，`rangeCount === 1`，
 等 3 秒后依然在），但**屏幕上一个像素的高亮都没有**。

→ `::selection` 在 Android WebView 上只在「触摸激活」期间由系统层绘制，
  手指一抬就没了。改 `::selection` 的样式救不了这个。

✅ 所以改用真实 DOM：选中的词加 `.is-selected`，由 `::before`
   在 `--sel-from` / `--sel-to` 两个比例之间画一个真实背景块。
   高亮现在常驻可见，也能被后面「建立区域」复用。

#### 顺带修掉三个真 bug

1. **高亮涂抹整行** —— 第一版给选中的行加背景 → 铺满整个词盒。
   实测拖 150px 时 7 行全被涂成横条。真因是**按词下标遍历**而不是按行，
   且行内每个词都按"整行比例"算自己的 `--sel-*`。现在按 `data-row`
   分组、每词按自己在行内的位置与选区求交。

2. **y 容差过大导致跨行** —— 原来是 `max(行高, 6) * 0.9`（行高 8px
   时容差 7.2px，而论文行距只有 9px），相邻行也被算进来。
   改成 `行高 * 0.5`（下限 3px），并把 score 里 y 的权重从 2 提到 20。

3. **`reader.js` 结构被编辑工具吃掉** —— `paintHighlight` 的函数头
   在一次替换中丢失，导致整个文件在 WebView 里
   `SyntaxError: Unexpected token 'function'`（第 1313 行），
   `ScholariusReader` 从未定义 → **点文献卡片毫无反应**。
   ⚠️ `node --check` 也报了同样的错，但当时漏跑。
   **现在每次改完 JS 必跑。**
...
源码 5294→5356 行之间加的函数（`getTextMarks`）**一个都没进 APK**。
症状是"新功能完全没生效"，看起来像代码写错了。

**现在每次改 assets 必做**：
```
gradle :app:clean -q ; gradle assembleDebug -q
# 然后解出 APK 里的文件，与源码比行数 / grep 这次新加的函数名
```
...
| 脚本 | 结果 |
|---|---|
| `tools/emu_region.py` | ✅ 冷启动 → 点卡片 → 点页面唤起菜单 → 点眼睛 → 横拖选字 → FAB 出现 → 选类型 → `textMarks` 写入 `{from:48,to:48,type:heading,level:1}` → FAB 收起 + 高亮清除 |
| 高亮像素 | ✅ 单条带 `y=1709~1729`（高 21px = 一行），`x=199~594`（宽 396px ≈ CSS 151px = 拖拽长度） |
| `tools/emu_gesture_matrix.py` | ✅ 横选字 / 纵滚动 / 斜向选字 |
