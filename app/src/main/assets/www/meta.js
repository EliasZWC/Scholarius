    ...
    var COMMON_FIELDS = [
        ...
        },
        {
            /*
              ...
              ⚠️ kind = 'date'：外表是 **8 个单字格占满一行**
                 （YYYYMMDD 逐位，用户要求）。

                 实现上是「一个铺满整行的透明 input + 8 个纯展示的
                 span」——**不是** 8 个真 input。原因见 detail.js
                 buildDateField 的长注释（真机软键盘不给字符 keydown，
                 多格方案的推进逻辑会整行卡死）。
                 ...
                 格各自可空，所以「只填年」「年+月」「完整日期」
                 「全不填」天然都支持 —— 粒度由「填到第几位」决定
                 （见 detail.js 的 formatDateValue）。
            */
            key: 'year',
